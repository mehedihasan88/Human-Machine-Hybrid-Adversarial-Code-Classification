...
GRAD_ACCUM = 2
LR = 1e-5
NUM_EPOCHS = 3                    # Total epochs (1 initial + 2 after hard mining)
WARMUP_RATIO = 0.1
MAX_GRAD_NORM = 1.0
...
HARD_MINING_MIN_CONF = 0.50       # LOWERED: More inclusive (was 0.65)
HARD_MINING_FOCUS_LABELS = [HYBRID_LABEL_ID, ADV_LABEL_ID]
HARD_MINING_INCLUDE_LOW_CONF = True  # NEW: Include low-confidence correct predictions
HARD_MINING_LOW_CONF_THRESHOLD = 0.70  # NEW: Include correct but uncertain (<0.70)
...
# Training arguments (Epoch 1 - train only 1 epoch)
training_args_epoch1 = TrainingArguments(
    output_dir="./results",
    num_train_epochs=1,  # Train only 1 epoch initially
...
if DO_HARD_MINING and NUM_EPOCHS > 1:
    ...
    for idx, row in tqdm(train_df.iterrows(), total=len(train_df), desc="Mining"):
        ...
        # Case 1: Misclassified focus labels with confidence threshold
        if is_focus_label and is_misclassified and max_prob >= HARD_MINING_MIN_CONF:
            loss = -np.log(true_label_prob + 1e-8)
            hard_negatives.append({
                ...
                'loss': loss,
                'tok_len': tok_len,
                'type': 'misclassified_focus'
            })
        # Case 2: Low-confidence correct predictions (uncertain cases) - focus labels only
        elif HARD_MINING_INCLUDE_LOW_CONF and is_focus_label and not is_misclassified:
            if max_prob < HARD_MINING_LOW_CONF_THRESHOLD:
                loss = -np.log(true_label_prob + 1e-8)
                hard_negatives.append({
                    'idx': idx,
                    'code': code,
        ...
        # Case 3: Misclassified non-focus labels with relaxed threshold
        elif is_misclassified and not is_focus_label:
            # Lower threshold for non-focus labels
            threshold = HARD_MINING_MIN_CONF - 0.15
    ...
    # Select top mined_count (prioritizing focus labels)
    # First, ensure we get focus label hard negatives
    focus_hard = [h for h in hard_negatives if h['label'] in HARD_MINING_FOCUS_LABELS]
    other_hard = [h for h in hard_negatives if h['label'] not in HARD_MINING_FOCUS_LABELS]
    
    # Take up to 70% from focus labels, rest from others
    focus_count = min(len(focus_hard), int(mined_count * 0.7))
    other_count = min(len(other_hard), mined_count - focus_count)
    
    selected_hard = focus_hard[:focus_count] + other_hard[:other_count]
    
    # If still not enough, fill from remaining sorted list
    if len(selected_hard) < mined_count:
        remaining = [h for h in hard_negatives if h not in selected_hard]
        selected_hard.extend(remaining[:mined_count - len(selected_hard)])
    ...
    # Update trainer with new dataset and remaining epochs
    trainer.train_dataset = train_dataset_epoch2
    trainer.class_weights = class_weights_epoch2 if USE_CLASS_WEIGHTS else None
    
    # Update training args for remaining epochs
    remaining_epochs = NUM_EPOCHS - 1  # Already trained 1 epoch
    trainer.args.num_train_epochs = remaining_epochs
    
    # Train remaining epochs (2, 3, etc.)
    print("\n" + "="*70)
    print(f"TRAINING EPOCHS 2-{NUM_EPOCHS}")
    print("="*70)
    trainer.train()
    trainer.save_model()
    tokenizer.save_pretrained("./results")
    print(f"Epochs 2-{NUM_EPOCHS} completed!")
else:
    if NUM_EPOCHS > 1:
        # Update training args for remaining epochs
        remaining_epochs = NUM_EPOCHS - 1  # Already trained 1 epoch
        ...
        print("\n" + "="*70)
        print(f"TRAINING EPOCHS 2-{NUM_EPOCHS} (NO HARD MINING)")
        print("="*70)
        trainer.train()
        trainer.save_model()
        tokenizer.save_pretrained("./results")
