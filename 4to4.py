#!/usr/bin/env python3
"""
Task C: Hybrid Code Detection with Direct 4-Class Classification
...
# Data / Subset
USE_SUBSET = True
SUBSET_TOTAL_N = 200_000        # REAL RUN: 200k or 500k for full training
SUBSET_SEED = 42
...
# Validation control
USE_FULL_VALIDATION = True          # True for real run (uses full validation set)
VAL_DRY_RUN_MAX_N = 10_000           # cap validation size in dry run
VAL_SUBSET_SEED = 123
...
GRAD_ACCUM = 2
LR = 2e-5
NUM_EPOCHS = 2                    # DRY RUN still uses 2 epochs to test mining
WARMUP_RATIO = 0.05
MAX_GRAD_NORM = 1.0
...
# Logging / Eval
LOGGING_STEPS = 1000            # REAL RUN: log every 1000 steps (adjust based on dataset size)
SAVE_TOTAL_LIMIT = 2
...
# Dynamic mining size (important)
HARD_MINING_FRAC = 0.20           # fraction of subset to mine
HARD_MINING_MAX_CAP = 20_000      # upper cap for large runs

# For DRY RUN (10k): mined_count = min(2000, int(SUBSET_TOTAL_N * HARD_MINING_FRAC))
# For REAL RUN: mined_count = min(HARD_MINING_MAX_CAP, int(SUBSET_TOTAL_N * HARD_MINING_FRAC))

# Test prediction / submission
GENERATE_SUBMISSION = True   # Set to True to generate submission.csv after validation
TEST_PARQUET_PATH = "/kaggle/input/sem-eval-2026-task-13-subtask-c/Task_C/test.parquet"
...
# ==================================================
# SECTION B — DATA LOADING + TOKEN LENGTH STATS
# ==================================================

# Load train and validation
train_df = pd.read_parquet('/kaggle/input/sem-eval-2026-task-13-subtask-c/Task_C/train.parquet')
val_df = pd.read_parquet('/kaggle/input/sem-eval-2026-task-13-subtask-c/Task_C/validation.parquet')
...
if not USE_FULL_VALIDATION:
    print("\n" + "="*70)
    print("[DRY RUN] LIMITING VALIDATION SET")
    print("="*70)
    ...
    print("\n" + "="*70)
    print("[REAL RUN] USING FULL VALIDATION SET")
    print("="*70)
    print(f"Validation samples used: {len(val_df)}")
...
if DO_HARD_MINING and NUM_EPOCHS > 1:
    ...
    # Calculate mined_count
    if SUBSET_TOTAL_N <= 10_000:
        # DRY RUN: smaller scale
        mined_count = min(2000, int(SUBSET_TOTAL_N * HARD_MINING_FRAC))
    else:
...
# ==================================================
# SECTION I — TEST PREDICTION + SUBMISSION
# ==================================================
...
def predict_test_and_write_submission(test_parquet_path, output_csv_path, model, tokenizer, device):
    ...
    # Stream test parquet
    ds = load_dataset("parquet", data_files=test_parquet_path, split="train", streaming=True)
    ...
    if os.path.exists(output_csv_path):
        ...
        print("\nIn Kaggle:")
        print("  → Go to 'Output' tab in the notebook")
        print("  → Find 'submission.csv' in the file list")
        print("  → Click the download button (⬇️) next to the file")
        print("="*70)
    else:
...
# ==================================================
# SECTION J — FINAL RUN MODES
# ==================================================
#
# DRY RUN (DEBUG MODE):
# - SUBSET_TOTAL_N = 10_000
# - NUM_EPOCHS = 2
# - HARD NEGATIVE MINING ENABLED (small scale: ~2000 mined)
# Goal:
#   - verify mining logic
#   - verify dataset rebuild
#   - verify training stability
#
# REAL RUN:
# - SUBSET_TOTAL_N = 200_000 or 500_000
# - NUM_EPOCHS = 2
# - HARD NEGATIVE MINING ENABLED (full scale: up to 20k mined)
# Goal:
#   - maximize Hybrid & Adversarial F1
#
# IMPORTANT GUARANTEES:
# - No sliding windows anywhere
# - Long code handled via multi-view cropping
# - Hard negative mining is ALWAYS exercised (dry run + real run)
# - Evaluation reflects real inference
# - Notebook runs end-to-end on Kaggle without manual intervention
#
